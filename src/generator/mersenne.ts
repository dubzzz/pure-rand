...
// jump() uses these coefficients to skip 2^128 values at once, creating independent streams.
// Each char encodes 6 bits of the jump polynomial from numpy's mt19937-jump.h.
// To regenerate from numpy's poly_coef (uint32[]) array:
// >  const bits = poly_coef.flatMap(v => Array.from({length: 32}, (_, i) => (v >>> i) & 1));
// >  const encoded = String.fromCharCode(...Array.from({length: Math.ceil(bits.length / 6)}, (_, i) => {
...
class MersenneTwister implements JumpableRandomGenerator {
  // Declared with `declare` and assigned in the constructor body rather than via
  // parameter properties. An uninitialized class field (`index;`) is first defined as
  ...
  // fields as their intended types from the very first write.
  declare private states: number[]; // states: between -0x80000000 and 0x7fffffff
  declare private index: number;
  constructor(states: number[], index: number) {
    this.states = states;
    this.index = index;
  }
