...
describe('purify', () => {
  it('should not alter the original instance (RandomGenerator)', () => {
    // Arrange
  ...
  it('should not alter the original instance (JumpableRandomGenerator)', () => {
    // Arrange
    const notImplemented = vi.fn();
    const clonedRng: JumpableRandomGenerator = {
      clone: notImplemented,
      getState: notImplemented,
      next: notImplemented,
      jump: notImplemented,
    };
    const rng: JumpableRandomGenerator = {
      clone: () => clonedRng,
      getState: notImplemented,
      next: notImplemented,
      jump: notImplemented,
    };
    const action = vi.fn<(rng: JumpableRandomGenerator, label: string, num: number) => string>(
      (_, label, num) => `${label}::${num}`,
    );
    ...
    const pureAction = purify(action) satisfies (
      ...
      label: string,
      num: number,
    ) => [string, JumpableRandomGenerator];
    const [out, outRng] = pureAction(rng, 'label', 10);
